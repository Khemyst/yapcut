...
4. The human briefs Claude: stream context, target output, tone, priority moments, preset selection
5. Claude reads the transcript, energy data, and chat log to understand the content
6. They discuss — Claude proposes segments, the human refines
7. Claude generates an **FCP 7 XML** file — either markers, post-ready cut, or both
8. Claude saves the XML in the **same folder as the source media** (never in `output/`), using collision-safe naming
9. Claude runs `python tools/validate_xml.py path/to/generated.xml` to validate
10. The human imports the XML into Premiere Pro
...
5. Read `input/chat.json` if available
6. Conduct the briefing conversation
7. Propose segments with types, timestamps, and reasoning
8. Discuss and refine with the human
9. Generate the FCP XML:
...
10. Save to source media folder (collision-safe naming) and run `python tools/validate_xml.py path/to/generated.xml`
11. Share validation results and file path
